...
**Ответ:**
- Успех: `audio/wav` — бинарные данные WAV-файла
- Ошибка (400/500): `text/plain` — текст ошибки

**Особенности:**
- Поддерживает SSML-разметку для расширенного контроля над синтезом
- Возвращает один полноценный WAV-файл

---

### `POST /restart`

Перезапускает TTS сервис (очищает кэш, перезагружает модель).
